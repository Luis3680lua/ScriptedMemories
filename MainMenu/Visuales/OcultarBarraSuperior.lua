local CONFIG = {
    Name = "Ocultar ícono, chat y micrófono",
    Description = "Oculta el ícono de Roblox, el chat y el micrófono. Nota: por una limitación de Roblox, esto también oculta la mochila y la lista de jugadores, ya que comparten la misma barra.",
    SettingKey = "visuals_hide_topbar_enabled",
    Default = false
}
...
local function applyTopbarSetting(enabled)
    pcall(function()
        StarterGui:SetCore("TopbarEnabled", not enabled)
    end)
    pcall(function()
...
if Menu.Settings[CONFIG.SettingKey] then
    task.spawn(applyTopbarSetting, true)
end
