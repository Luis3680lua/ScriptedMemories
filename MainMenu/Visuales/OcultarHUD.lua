local CONFIG = {
    Name = "Ocultar HUD del juego",
    Description = "Oculta toda la interfaz del juego (vida, minimapa, indicadores, etc.) sin afectar este menú.",
    SettingKey = "visuals_hide_hud_enabled",
    Default = false,
    MenuGuiName = "ScriptedMemoriesUI"
}
...
local function isOwnGui(gui)
    return gui.Name == CONFIG.MenuGuiName
end

local function hideGui(gui)
    if isOwnGui(gui) then return end
    if hiddenGuis[gui] ~= nil then return end
...
local function applyHudSetting(enabled)
...
if Menu.Settings[CONFIG.SettingKey] then
    task.spawn(applyHudSetting, true)
end
